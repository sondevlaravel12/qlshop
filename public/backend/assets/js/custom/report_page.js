...
//   jQuery.getJSON( url [, data ] [, success ] ): Load JSON-encoded data from the server using a GET HTTP request.
  $.getJSON('api/report/revenue/daily-in-month', function(response) {
        chart.updateSeries([{
            name: 'doanh thu',
            data: response
        }])
    });
$('#revenue-chart').on('change',function(){
    $selectFillter = $(this).val();
    if($selectFillter =='this_week'){
        $.getJSON('api/report/revenue/daily-in-week', function(response) {
            chart.updateSeries([{
                name: 'doanh thu',
                data: response
            }])
        });
    }else if($selectFillter =='this_month'){
        $.getJSON('api/report/revenue/daily-in-month', function(response) {
            chart.updateSeries([{
                name: 'doanh thu',
                data: response
        ...
        $.getJSON('api/report/revenue/monthly', function(response) {
            chart.updateSeries([{
                name: 'doanh thu',
                data: response
            }])
